...
void Executor::setTheSame(Expr* src, Expr* dest) {
    std::lock_guard<std::mutex> _l(mMutex);
    auto solution = mSolutions.find(src);
    if (solution == mSolutions.end()) {
        mSolutions.erase(dest);
        return;
    }
    mSolutions[dest] = solution->second;
}
